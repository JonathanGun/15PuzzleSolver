        ...
        while not self.isSolved():
            for direction in Direction:
                ...
                if self.isValidMove(i, j):
                    tmp = Node(self.curNode.grid, self.curNode.empty, self.curNode, direction.value)
                    if not self.hasNode(tmp.grid):
                        self.nodes.append(tmp)
                        heappush(self.activeNode, (tmp.cost, len(self.nodes) - 1))
            cost, itmp = heappop(self.activeNode)
            self.curNode = self.nodes[itmp]
        return True
